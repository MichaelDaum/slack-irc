...
class Bot {
  constructor(options) {
    ...
    this.avatarOfUser = options.avatarOfUser || {};
    ...
    this.muteMessages = {
      slack: [],
      irc: [],
      ...options.muteMessages
    };

    // compile regexps
    if (this.muteMessages.slack.length) {
      this.muteMessages.slackRegExp = new RegExp(this.muteMessages.slack.join('|'), 'i');
    }
    if (this.muteMessages.irc.length) {
      this.muteMessages.ircRegExp = new RegExp(this.muteMessages.irc.join('|'), 'i');
    }
  ...
  sendToSlack(author, channel, text) {
    ...
    if (slackChannelName) {
      ...
      // muted messages
      if (!mute && this.muteMessages.ircRegExp && this.muteMessages.ircRegExp.test(text)) {
        mute = true;

        // mute user completely
        if (this.muteUsers.irc.indexOf(author) === -1) {
          this.muteUsers.irc.push(author);
        }
      }
      ...
      let iconUrl;
      const normalizedAuthorName = author.replace(/_+$/, '');
      if (this.avatarOfUser[normalizedAuthorName]) {
        iconUrl = this.avatarOfUser[normalizedAuthorName];
      } else if (normalizedAuthorName !== this.nickname && this.avatarUrl) {
        iconUrl = this.avatarUrl.replace(/\$username/g, normalizedAuthorName);
      }
